...
### 2.1. Dataset de registros do GBIF e speciesLink
...
Finalmente, Vamos linkar as bases SQL ao ambiente de trabalho.


**Observação:** os arquivos `.db` baixados **devem impreterivelmente estar no mesmo diretório de trabalho** que você selecionou no início deste tutorial. Certifique-se disso!
...
```{r eval=FALSE, include = T}
read.delim(choose.files()) -> flora2020 ##selecione o arquivo "fb2020.txt" que você baixou no seu diretório
...
### 4.1. Busca no repositório do GBIF
...
search <- "Serra do Mar" ## ATENÇÃO: altere livremente o topônimo da sua busca aqui!!! Neste caso, para fins de exemplo estou usando "Serra do Mar", mas note que poderia ser qualquer outro topônimo!
...
### 4.2. Busca no repositório do speciesLink
...
## Apenas para fins de análise, vamos criar um elemento que informe quantos registros foram recuperados, e para quantas espécies e famílias.
suma.raw_splink <- c(nrow(selected_splink), nrow(unique(selected_splink$scientificname)), nrow(unique(selected_splink$family)))

```


## 5. Buscas na base do GBIF
...
## Loop para realizar as buscas dos taxonKeys online - pode demorar vários minutos:
...
## Para fins de visualização de dados, vamos novamente criar um vetor com as informações de registros, espécies e famílias existentes na base de dados após o cruzamento e padronização com a base da Flora 2020...:
...
write.xlsx(output_final, paste("4_final_gbif_selected_", search, ".xlsx", sep = ""), append = T)

## Para sumarizar os resultados:

suma.final_gbif <- c(nrow(output_final), length(unique(output_final$`5. Espécie`)), length(unique(output_final$`3. Família`)))

```


## 6. Buscas na base do speciesLink


### 6.1. Cruzamento com os dados da Flora do Brasil 2020
...
merge(flora2020, selected_splink, by.y = "scientificname", by.x = "name_concat", all.y = T) -> data_merged
data_merged <- data_merged[!(is.na(data_merged$id)),]
data_merged[1,] <- 1
